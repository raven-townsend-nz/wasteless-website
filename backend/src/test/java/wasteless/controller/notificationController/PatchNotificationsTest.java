...
class PatchNotificationsTest {
    ...
    @Test
    @WithMockUser
    void markNotificationAsRead_withCorrectUser_sends200() throws Exception {
        when(authUtil.getCurrentUser()).thenReturn(user1);
        mockMvc
                .perform(MockMvcRequestBuilders.patch("/notifications/" + user1.getUserId() + "/read/" + notification.getId()))
